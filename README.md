...
- **预览模式**：手机尺寸（375px 居中）iframe 模拟真实阅读器：整书连续阅读、封面、目录跳转、章节切换（含模板样式）。
- **章节管理**：支持在光标处拆分章节、合并下一章、上下箭头与拖拽重排目录。
- **全局查找替换**：全书正文 + 章节标题查找替换，保留排版标签，可区分大小写。
- **TXT / Markdown 导入**：自动按章节标记切分，并转换为可编辑 EPUB 章节。
