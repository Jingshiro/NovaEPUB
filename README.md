...
你说它像 Sigil？行吧，像，但是长得是人用的那种。没有账号、没有云端、没有安装，所有数据都在你自己的浏览器里——我连你书里写了啥都不知道，也懒得知道。
...
- ☁️ **云同步（可选）**：WebDAV（默认给你填好坚果云地址）或 S3 兼容（AWS / B2 / R2 / MinIO 都行）。整库快照 + latest，云端留最近 10 份。密码 / 密钥只存本机，不传给我（我也没服务器收）。
...
**明确不搞**：导出 mobi / azw3（没人用）、插件系统（克制是美德）、小程序云同步（wx.request 域名白名单，死路，别问）。
